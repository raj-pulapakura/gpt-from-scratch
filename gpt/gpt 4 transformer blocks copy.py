...
CONTEXT_LEN = 8 # number of previous chars used to predict next char
N_EMBED = 32 # token embedding dimension
NUM_HEADS = 4 # number of heads in multi-head attention
HEAD_SIZE = 128 # embedding dimension outputted by multi-head attention
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, input_dim, embed_per_head):
        super().__init__()
        self.heads = nn.ModuleList([AttentionHead(input_dim, embed_per_head) for _ in range(NUM_HEADS)])
...
class Block(nn.Module):
    ...
    def __init__(self, input_dim, output_dim):
        super().__init__()
        embed_per_head = output_dim // NUM_HEADS
        self.sa = MultiHeadAttention(input_dim, embed_per_head)
        self.ffwd = FeedForward(output_dim)

    def forward(self, x):
        x = self.sa(x) # (B, T, output_dim)
        x = self.ffwd(x) # (B, T, output_dim)
        return x
